...
const USD_TO_INR = 83 // keep this in sync with your ProductPrice conversion

// Base ranges are in USD (these are what the backend expects in the query -> "value")
// We build a display `name` in INR by multiplying by USD_TO_INR
const baseUsdRanges = [
  { min: 1, max: 20 },
  { min: 21, max: 50 },
  { min: 51, max: 1000 },
...
// helper to format INR with no decimals
...
// prices: value (USD min-max) stays in USD so filters work; name (display) shows INR
const prices = baseUsdRanges.map(({ min, max }) => ({
  name: `${formatINR(min * USD_TO_INR)} to ${formatINR(max * USD_TO_INR)}`,
  value: `${min}-${max}`,
}))
...
export default async function SearchPage(props: {
  ...
  // friendly label for the selected price filter (shows INR name if available)
  const selectedPriceLabel =
    price !== 'all' ? prices.find((p) => p.value === price)?.name ?? price : null
    ...
    <div>
      <div className='mb-2 py-2 md:border-b flex-between flex-col md:flex-row '>
        <div className='flex items-center'>
          ...
          {category !== 'all' && category !== '' && `  Category: ` + category}
          {tag !== 'all' && tag !== '' && `   Tag: ` + tag}
          {price !== 'all' && `    Price: ` + (selectedPriceLabel ?? price)}
          {rating !== 'all' && `   Rating: ` + rating + ` & up`}
          &nbsp;
          {(q !== 'all' && q !== '') ||
          (category !== 'all' && category !== '') ||
